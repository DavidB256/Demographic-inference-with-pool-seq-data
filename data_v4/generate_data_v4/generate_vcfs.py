...
if __name__ == "__main__":
    ...
    for subdirectory in ["vcfs/", "popinfos/"]:
        if not os.path.exists(yd["pipeline_params"]["output_dir"] + subdirectory):
            os.makedirs(yd["pipeline_params"]["output_dir"] + subdirectory)
    ...
    for sample_size in yd["pipeline_params"]["sample_sizes"]:
        ...
        popinfo_file = f"{yd['pipeline_params']['output_dir']}popinfos/n{sample_size}_popinfo.txt"
        ...
        for rep in range(yd["pipeline_params"]["num_of_demography_sim_replicates"]):
            ...
            with open(f"{yd['pipeline_params']['output_dir']}tss/n{sample_size}_mseed{seed}.ts", "w") as f:
                mts.dump(f)
            # Export VCF created from TreeSequence "mts"
            with open(f"{yd['pipeline_params']['output_dir']}vcfs/n{sample_size}_mseed{seed}.vcf", "w") as f:
                mts.write_vcf(f)